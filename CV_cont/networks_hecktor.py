...
def DeepMTS(vol_size, Clinic_size):
    ...
    Survival_pred = Surv_head(x_in1, x_in2, Clinic, L2_reg=0.1, num_node=64)
    
    return Model(inputs=[PET, CT, Clinic], outputs=[Seg_pred, Classifier_pred, Survival_pred])
...
def Surv_head(x_in1, x_in2, Clinic, L2_reg, num_node, droprate=0.5):
    ...
    x_concat1 = BatchNormalization()(x_concat1)
    ...
    x_concat2 = BatchNormalization()(x_concat2)
...
def Classifier_net(x_in, droprate=0.05):
    ...
    ################################################################################
    x = Dropout(droprate)(x_out3)
    x = GlobalAveragePooling3D()(x)
    Classifier_pred = Dense(1, activation="sigmoid", name="Classifier")(x)
    ################################################################################

    return x_out1, x_out2, x_out3, Classifier_pred
